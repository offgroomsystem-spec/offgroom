...
const Agendamentos = () => {
  ...
  const handlePacoteSubmit = async (e: React.FormEvent) => {
    ...
    if (clienteCadastro && clienteCadastro.nome_cliente.trim() !== pacoteFormData.nomeCliente.trim()) {
      toast.error(`O número de WhatsApp informado pertence ao cliente "${clienteCadastro.nome_cliente}", mas o nome preenchido é "${pacoteFormData.nomeCliente}". Corrija antes de salvar.`);
      return;
    }
