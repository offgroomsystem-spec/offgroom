...
import { TrendingDown, Calendar, Wrench, FileText, Filter, Download, Edit2, Trash2, ChevronDown, ChevronUp, AlertCircle } from "lucide-react";
...
export function DespesasNaoOperacionais() {
  ...
  const loadData = async () => {
    try {
      ...
      const { data: lancamentosData, error: lancamentosError } = await supabase
        ...
        .eq("user_id", user.id)
        .eq("tipo", "Despesa")
        .eq("descricao1", "Despesa Não Operacional")
  ...
  const manutencaoReparos = useMemo(
    () =>
      lancamentosFiltrados
        .filter((l) =>
          l.itens.some((i) =>
  ...
  const outrasNaoOperacionais = useMemo(
    () =>
      lancamentosFiltrados
        .filter((l) => l.itens.some((i) => i.descricao2 === "Outras Despesas Não Operacionais"))
        .reduce((acc, l) => acc + l.valorTotal, 0),
    [lancamentosFiltrados]
  );
  ...
  const dadosGraficoBarras = useMemo(() => {
    ...
    return categorias.map((cat) => ({
      categoria: cat === "Outras Despesas Não Operacionais" ? "Outras" : cat,
  ...
  const handleEditar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const itensParaInserir = itensLancamento.map((item) => ({
        ...
        descricao2: item.descricao2,
        valor: item.valor,
        produto_servico: item.observacao || null,
      }));
  ...
  const exportarCSV = () => {
    ...
    link.setAttribute("download", `despesas-nao-operacionais-${new Date().toISOString().split("T")[0]}.csv`);
    link.click();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {mostrarFiltros && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Refine sua busca por despesas não operacionais</CardDescription>
          </CardHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-blue-50 border-blue-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-blue-700 mt-1">Média mensal de gastos não operacionais</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-orange-50 border-orange-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{formatarMoeda(outrasNaoOperacionais)}</div>
            <p className="text-xs text-orange-700 mt-1">Outras despesas não operacionais</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lançamentos de Despesas Não Operacionais</CardTitle>
          <CardDescription>
          ...
          {lancamentosFiltrados.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-lg font-semibold text-muted-foreground">
                Nenhum lançamento de Despesa Não Operacional encontrado
              </p>
      ...
      {lancamentosFiltrados.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Evolução das Despesas Não Operacionais por Dia</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-red-900">
                  Total de Despesas Não Operacionais no Período:
                </span>
              </div>
      ...
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Editar Lançamento de Despesa Não Operacional</DialogTitle>
